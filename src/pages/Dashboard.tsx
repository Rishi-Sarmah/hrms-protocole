      ...
      <main className="container mx-auto p-4 mt-6">
        ...
        {loading ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {reports.map((report) => (
              <div key={report.id} className="p-4 bg-white rounded-xl shadow-lg border-2 border-slate-200 hover:shadow-xl transition text-xs">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-sm font-bold truncate pr-2" title={report.filename}>
                    {report.filename}
                  </h3>
                </div>
                <p className="text-[11px] text-gray-400 mb-3">{new Date(report.createdAt).toLocaleString()}</p>
                <div className="mt-2">
                  <pre className="p-2 text-[11px] bg-gray-50 rounded border border-gray-100 overflow-auto max-h-40">{JSON.stringify(report.indicators, null, 2)}</pre>
                </div>
              </div>
            ))}
          </div>
