...
exports.backfillEmbeddings = exports.analyzeReport = exports.chat = exports.generateSessionEmbedding = void 0;
...
exports.generateSessionEmbedding = (0, firestore_1.onDocumentWritten)({
    ...
    try {
        ...
        const text = (0, serialize_1.serializeSession)(sessionData);
        ...
        console.log(`Session ${event.params.sessionId}: embedding generated (${embeddingVector.length} dims, text: ${text.length} chars)`);
    }
    catch (error) {
...
exports.chat = (0, https_1.onCall)({
    ...
    // Auth check
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "You must be signed in to use the chat.");
    }
    const { question, history = [], language = "en" } = request.data;
    if (!question || typeof question !== "string" || question.trim().length === 0) {
        throw new https_1.HttpsError("invalid-argument", "A question is required.");
    }
    ...
    for (const doc of querySnapshot.docs) {
        ...
        const text = data.embeddingText || (0, serialize_1.serializeSession)(data);
...
exports.analyzeReport = (0, https_1.onCall)({
    secrets: [geminiApiKey],
    memory: "512MiB",
    timeoutSeconds: 120,
    enforceAppCheck: false,
    ...
    if (!data) {
        throw new https_1.HttpsError("invalid-argument", "Session data is required.");
    }
    const ai = new genai_1.GoogleGenAI({ apiKey: geminiApiKey.value() });
    const prompt = `
  ...
  `;
    try {
        ...
        throw new https_1.HttpsError("internal", "An error occurred while communicating with the AI service. Please try again.");
    }
});
...
exports.backfillEmbeddings = (0, https_1.onRequest)({
    secrets: [geminiApiKey],
    memory: "1GiB",
    timeoutSeconds: 540,
}, async (req, res) => {
    ...
    for (const doc of snapshot.docs) {
        ...
        try {
            const text = (0, serialize_1.serializeSession)(data);
            if (!text || text.length < 20) {
                skipped++;
                continue;
            }
