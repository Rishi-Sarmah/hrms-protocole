...
interface OperatingDataRow {
  category: string;
  subcategory: string;
  volume: { kgs: number; m3: number; litre: number };
  value: { cif: number; fob: number; marchande: number };
  occFees: number;
  result: number;
}
...
function executionRate(forecast: number, achievement: number): string {
  if (forecast === 0) return "N/A";
  return `${((achievement / forecast) * 100).toFixed(1)}%`;
}

export function serializeSession(doc: SessionData): string {
  const parts: string[] = [];
  ...
  if (data.staff && data.staff.length > 0) {
    ...
    // Group by category
    const categories: Record<string, { male: number; female: number }> = {};
    for (const row of data.staff) {
      const cat = row.category || "Other";
      if (!categories[cat]) categories[cat] = { male: 0, female: 0 };
    ...
    parts.push(
      `Personnel: ${totalStaff} total staff (${totalMale} male, ${totalFemale} female). ` +
  ...
  if (data.budget) {
    ...
    const summarizeBudgetSection = (
      ...
      const topItems = rows
        ...
        .map(
          (r) =>
            `${r.label}: forecast ${formatNumber(r.forecast)}, achieved ${formatNumber(r.achievement)} (${executionRate(r.forecast, r.achievement)})`
        )
        .join("; ");
      return (
        `${label}: total forecast ${formatNumber(totalForecast)}, ` +
    ...
    // Treasury
    const receiptsTotal = (b.treasuryReceipts || []).reduce(
      (s, r) => s + (r.achievement || 0),
      0
    );
    const disbursementsTotal = (b.treasuryDisbursements || []).reduce(
      (s, r) => s + (r.achievement || 0),
      0
    );
    ...
    parts.push(
      `Budget: ${prodSummary} ${chargesSummary} ` +
  ...
  if (data.exploitation) {
    ...
    if (ex.labAnalysis && ex.labAnalysis.length > 0) {
      ...
      const totalNonCompliant = ex.labAnalysis.reduce(
        (s, r) => s + (r.nonCompliant || 0),
        0
      );
      ...
      const labDetails = ex.labAnalysis
        ...
        .map(
          (r) =>
            `${r.product || r.category}: ${r.received} received, ${r.analyzed} analyzed, ${r.nonCompliant} non-compliant`
        )
        .join("; ");

      parts.push(
        `Lab Analysis: ${totalReceived} total samples received, ${totalAnalyzed} analyzed, ` +
